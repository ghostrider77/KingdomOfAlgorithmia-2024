  ...
  let rec create_tree = function
    ...
    | name ->
        match StringMap.find_opt name adjacency_list with
          | None -> Leaf {name}
          | Some neighbors ->
              let valid_neighbors = List.filter (fun n -> n <> "ANT" && n <> "BUG") neighbors in
              Node {name; children = List.map create_tree valid_neighbors}
  in create_tree "RR"
...
let calc_singular_length_path (tree : tree) : string =
  ...
  let key = fst @@ List.find (fun (_, length) -> length = unique_length) items in
  key |> String.split_on_char ',' |> List.map (fun s -> String.make 1 s.[0]) |> String.concat ""
