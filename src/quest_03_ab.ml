...
let calc_nr_earth_blocks ({mines; nr_rows; nr_cols} : grid) : int =
  let is_valid {x; y} = 0 <= x && x < nr_rows && 0 <= y && y < nr_cols in
  let get_neighbors {x; y} = [{x = x - 1; y}; {x; y = y + 1}; {x = x + 1; y}; {x; y = y - 1}] in
  let rec loop acc blocks =
    if CoordSet.is_empty blocks then acc
    else
      let is_inner_coord coord =
        let neighbors = List.filter is_valid (get_neighbors coord) in
